...
| **`redis/`** | Redis AOF persistence (`appendonly.aof`). |
| **`uploads/`** | User-uploaded files (riders, plots, logos, etc.) — used by the app container. |
...
If `DATA_DIR` is on a mounted disk, ensure the Docker user can write there. Postgres and Redis images expect to own their data dirs inside the container; host UID mapping may require `chown` or matching volume options — see Docker docs if restores fail to start.
