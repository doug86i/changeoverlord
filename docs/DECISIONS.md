...
8. **Defer `onOp` / `emitOp` out of React state updaters (planned):** In **`packages/react/src/components/Workbook/index.tsx`**, **`setContextWithProduce`** calls **`emitOp(result, filteredPatches, options)`** *inside* the **`setContext(function (ctx_) { … })`** functional updater after **`produceWithPatches`**. That is a **side effect in an updater** (same class of bug as Immer freeze vs replay, but for **collab**: duplicate **`onOp`**). **`handleUndo` / `handleRedo`** also call **`emitOp`** inside **`setContext`** updaters. React 18 Strict Mode and scheduling can surface **multiple** emissions for one gesture; each **`addSheet`** batch uses a **new sheet UUID**, so **server `addSheet` idempotency by `id`** does not dedupe. **Root fix (fork):** stash **`{ filteredPatches, options, undo }`** (or the built **`Op[]`**) in a ref and invoke **`onOp(…)` from `useLayoutEffect`** (or **`queueMicrotask`** after commit) **once per committed transition**, not from inside the updater. Prefer a small, isolated commit suitable for an **upstream PR** to `ruilisi/fortune-sheet`. **Belt-and-suspenders (this repo, optional):** a short **per-socket** relay cooldown for WebSocket batches containing **`addSheet`** if fork rebuild is delayed — see **[`docs/KNOWN_ISSUES.md`](KNOWN_ISSUES.md) §83**.
