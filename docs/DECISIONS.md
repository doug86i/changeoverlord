...
**`applyOp` echo suppression (this app):** FortuneSheet's `applyOp` method uses the **same `setContextWithProduce`** path as local user actions. `setContextWithProduce` captures Immer patches via `produceWithPatches` and fires the `onOp` callback with re-derived ops. Without suppression, every remote op that arrives via Yjs and is applied with `wb.applyOp(ops)` fires `onOp`, which pushes the echo ops **back** to Yjs — the other peer applies them again. For idempotent `replace` ops this is invisible; for `addSheet`/`deleteSheet` it creates duplicates. Fix: set `suppressYjsOpsRef.current = true` around the observe handler's `applyOp` calls, and wrap each `applyOp` in **`flushSync`** so React processes the state update synchronously while the suppress flag is still set (React 18 automatic batching would otherwise defer the updater and the `onOp` callback would fire after the suppress flag was cleared).
