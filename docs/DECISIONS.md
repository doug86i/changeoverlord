...
8. **Defer collab `onOp` out of React state updaters:** **`packages/react/src/components/Workbook/index.tsx`** — **`setContextWithProduce`**, **`handleUndo`**, and **`handleRedo`** used to call **`emitOp` / `onOp`** *inside* **`setContext`** functional updaters (a **side effect in an updater**). React can invoke those updaters more than once; each **`addSheet`** batch carries a **new sheet UUID**, so **server `addSheet` idempotency by `id`** does not dedupe duplicate emissions. **Fix:** queue **`{ ctx, patches, options, undo }`** on a ref and flush the queue from **`useLayoutEffect`** after commit (preserves order when multiple emits enqueue in one batch). Fork commit: *Defer collab onOp to useLayoutEffect (avoid duplicate addSheet)* — push **`dhsl/v1.0.4`** on **`doug86i/fortune-sheet`** after rebuilding **`vendor/fortune-sheet-react-1.0.4.tgz`**. Suitable for an **upstream PR** to `ruilisi/fortune-sheet`.
