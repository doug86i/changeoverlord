...
**Duplicate sheet tabs (mitigated):** Server **`addSheet`** idempotency plus structural **`fullState`** to peers; see **[`docs/KNOWN_ISSUES.md`](KNOWN_ISSUES.md) §83**. Persisted duplicates from older builds still need **Export → edit JSON → Import**.
