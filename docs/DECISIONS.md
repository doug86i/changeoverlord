...
**Open collab bug:** Duplicate **sheet tabs** on remotes after **Add sheet** can still occur **after the Yjs → relay refactor** — the relay persists **`sheets_json`**; duplicate **`onOp`** → duplicate server applies → **reload does not fix** persisted duplicates. See **[`docs/KNOWN_ISSUES.md`](KNOWN_ISSUES.md) § #83**.
