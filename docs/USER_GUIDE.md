...
- **Inline editing** — Click band name, start time, or end time to edit in place. When a slot’s **start** falls on a **later local calendar day** than the stage day (typical after-midnight acts), a yellow **+Nd** badge appears next to the start time (**+1d**, **+2d**, … — and on **Print**).
...
- **Your name** — The first time you open chat in this browser, you are asked for a **display name** (stored locally). You can change it under **Options** later. Messages need a non-empty name to send. On **iPhone/iPad Safari**, the name and message fields use text large enough that the browser should **not** zoom the whole page when you tap to type.
- **Stage context** — You see messages for **that stage** plus anything sent to the **whole event**. On the **event** detail page (`/events/…`), open **Options** and use the **Stage** dropdown to pick which thread you are viewing; on **stage**, **stage day**, **patch**, or **performance files** pages, the stage comes from the page you are on (no dropdown). **Your** messages appear in **bubbles on the right**; everyone else’s on the **left** (matched by your current **Name** in Options — two people using the exact same name will both see those messages as “yours”).
- **Sending** — **Enter** sends; **Shift+Enter** adds a new line (also noted in the message box placeholder). **Options** and **Who’s online** open a panel **above** those two buttons (so the buttons stay fixed above the message box). Each panel has a small heading when open. **Options** holds **Name**, **Stage** (when you opened the event from its detail page), and **This stage** vs **Whole event**. **Who’s online** lists other tabs that have chat open on this event (same app server); see **`docs/REALTIME.md`** (chat presence).
- **Open / minimize** — Tap **Chat** to open the full window; **Minimize** (or **click outside** / **Escape** after closing **Who’s online** or **Options**) returns to the small **Chat** button. **New messages** open the panel and **flash until you click or focus the dock** (including the **Chat** button when it is collapsed). If you close the panel without touching the dock, the **Chat** button keeps flashing. Your own sends do not flash the dock.
...
- **Who’s online (desktop/tablet)** — When **more than one** collab connection is open for this act, **Live** shows **N people editing** (everyone with the patch open counts, including your tab).
...
- **One clock face everywhere** — The **top** of the page shows the same **arena** layout performers see on a TV: band / changeover context, **hero countdown** (JS-sized to the available box), **local time**, and footer metadata (stage, date, pace, slot). **Typography scales with the clock panel** (not just the browser window), so small windows, split-screen, or the Event dashboard previews stay readable. **Below** that, a full-width **controls** area has **urgent message**, focus card, navigation, and the **schedule** list.
...
- **Fullscreen** — Press **F** or **Fullscreen** so the **clock arena** uses the whole screen (browser fullscreen); the controls below stay on the page but are hidden behind the fullscreen surface until you exit. **Fullscreen / Exit** sit on the **same row** as the stage / date / pace footer (compact, to save vertical space). While fullscreen is on, the footer shows **Exit fullscreen (F)** only (not both buttons). In supported browsers, fullscreen also asks the device to **stay awake** (screen wake lock) so phones and tablets do not sleep on the clock; if the browser does not support it, use the device’s own “keep display on” or power settings. Old bookmarks with **`?kiosk=1`** are redirected to the normal clock URL (query removed).
