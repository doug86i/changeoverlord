...
- **Changeover** — When the schedule is in a **gap between acts** (nobody on stage, next act not yet started), a **Changeover** banner appears — same condition and styling as on the **patch** sidebar. The **focus** card can show **changeover duration** between the previous act’s end and the focused act’s start when applicable.
