...
**Root fix (fork, shipped in vendor):** **`emitOp` / `onOp`** are deferred until **after** commit via **`useLayoutEffect`** + a ref queue — see **`docs/DECISIONS.md`** (FortuneSheet fork, bullet **#8**). Source: **`doug86i/fortune-sheet`** branch **`dhsl/v1.0.4`** (push after each fork commit — **AGENTS.md**); tarball **`vendor/fortune-sheet-react-1.0.4.tgz`** in this repo.

**Mitigation (current code):**

| Layer | Behaviour |
|--------|-----------|
| **`applyOpBatchToSheets`** ([`api/src/lib/workbook-ops.ts`](../api/src/lib/workbook-ops.ts)) | **`addSheet`**: if the new sheet’s **`id`** is already in the workbook, **skip**; otherwise **`ensureSheetDataMatrixFromRowCol`** (Fortune often omits **`data`** until client init). Full **`luckysheetfile`** **`replace`**: same per sheet. **`insertRowCol`** mirrors Fortune’s **`index` / `count` / `direction`** shape (not legacy **`start` / `end`**). |
| **FortuneSheet fork (`dhsl/v1.0.4`)** | **React:** `SheetTab` creates one stable `newSheetID` per click and passes it to `addSheet`. **Core:** `addSheet` exits when that id already exists. This closes replay paths where one click emitted several batches with different UUIDs. |
| **Collab relay** ([`api/src/plugins/collab-ws-relay.ts`](../api/src/plugins/collab-ws-relay.ts)) | After a successful apply, batches with **`addSheet`**, **`deleteSheet`**, or full **`luckysheetfile`** **`replace`** are pushed to **other** sockets as **`fullState`** (not **`op`**) so remotes align with **`room.sheets`**. Cell-only batches still use **`op`** broadcast. **Rapid duplicate addSheet guard:** drop same-socket `addSheet` bursts inside a short cooldown and send sender **`fullState`** to resync. **Cold room:** bootstrap from Postgres with **`sheetsSafeForCollabPersist`** + **`hydrateSheetsForCollabRoom`** — **`sheetsUsableForServing`** alone would reject valid persisted workbooks and substitute the blank **Input / RF** default after **Add sheet** + navigate away + return. |
| **`usePatchWorkbookCollab`** ([`web/src/lib/patchWorkbookCollab.ts`](../web/src/lib/patchWorkbookCollab.ts)) | **Incoming:** second and later **`fullState`** messages bump **`Workbook` `key`** so the grid remounts from server snapshots. **Outgoing:** every **`onOp`** batch is sent. |
| **Collab relay persist** | Debounced writes use **`sheetsSafeForCollabPersist`**, not **`sheetsUsableForServing`**, so cleared / sparse grids still persist. |

**Still not idempotent:** row/column structural paths in the editor can still misbehave if duplicate **`op`** delivery occurs without a **`fullState`** — **`insertRowCol`** server mirror now matches Fortune for **`data`** dimensions; reopen if edge cases remain.
