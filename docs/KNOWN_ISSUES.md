...
| Layer | Behaviour |
|--------|-----------|
| **`applyOpBatchToSheets`** ([`api/src/lib/workbook-ops.ts`](../api/src/lib/workbook-ops.ts)) | **`addSheet`**: if the new sheet’s **`id`** is already in the workbook, **skip**. **`insertRowCol`** mirrors Fortune’s **`index` / `count` / `direction`** shape (not legacy **`start` / `end`**). |
| **Collab relay** ([`api/src/plugins/collab-ws-relay.ts`](../api/src/plugins/collab-ws-relay.ts)) | After a successful apply, batches with **`addSheet`**, **`deleteSheet`**, or full **`luckysheetfile`** **`replace`** are pushed to **other** sockets as **`fullState`** (not **`op`**) so remotes align with **`room.sheets`**. Cell-only batches still use **`op`** broadcast. |
