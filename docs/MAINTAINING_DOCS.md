...
| **Patch template library** (Excel upload, **`POST /api/v1/patch-templates/blank`**, or duplicate; stage picks a stored template; Yjs WebSocket sync delivers workbook state) | **`examples/README.md`**, **`USER_GUIDE.md`** (templates), **`api/src/routes/v1/patch-templates.ts`**, **`api/src/lib/default-patch-sheets.ts`**, **`web/src/lib/patchWorkbookCollab.ts`**. |
