...
| **Built-in patch template presets** (blank layout + bundled seed example) | **`api/src/lib/patch-template-presets.ts`**, **`api/src/lib/seed-patch-templates.ts`**, **`USER_GUIDE.md`** (templates section), and **`POST /patch-templates/new`** in **`api/src/routes/v1/patch-templates.ts`**. |
