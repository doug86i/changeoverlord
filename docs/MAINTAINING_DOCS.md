...
| **File attachments** (riders/plots: upload, list, extract PDF page, delete) | **`api/src/routes/v1/files.ts`**, **`api/src/lib/pdf.ts`**, **`api/src/lib/pdf-thumbnails.ts`** (Poppler **`pdftoppm`** for extract previews), **`api/src/lib/upload-allowlists.ts`**, **`web/src/components/FileAttachments.tsx`**, **`USER_GUIDE.md`** (Files section), **`docs/REALTIME.md`** (`broadcastInvalidate` keys `files` / `files`+`performance`), **`docs/LICENSING.md`** (**pdf-lib**). |
