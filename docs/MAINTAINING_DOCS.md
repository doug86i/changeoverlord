...
| **Patch template library** (Excel or **JSON** upload, **`POST /api/v1/patch-templates/blank`**, or duplicate; stage picks a stored template; Yjs WebSocket sync delivers workbook state) | **`docs/PATCH_TEMPLATE_JSON.md`**, **`examples/README.md`**, **`USER_GUIDE.md`** (templates), **`api/src/routes/v1/patch-templates.ts`**, **`api/src/lib/json-patch-template.ts`**, **`api/src/lib/excel-to-sheets.ts`**, **`api/src/lib/default-patch-sheets.ts`**, **`web/src/lib/patchWorkbookCollab.ts`**. |
| **File attachments** (riders/plots: upload, list, convert to PDF, extract PDF page, delete) | **`api/src/routes/v1/files.ts`**, **`api/src/lib/pdf.ts`**, **`api/src/lib/pdf-thumbnails.ts`** (Poppler **`pdftoppm`**), **`api/src/lib/convert-to-pdf.ts`** (ImageMagick / LibreOffice / **pdf-lib**), **`api/src/lib/upload-allowlists.ts`**, **`web/src/components/FileAttachments.tsx`**, **`USER_GUIDE.md`** (Files section), **`docs/REALTIME.md`** (`broadcastInvalidate` keys `files` / `files`+`performance`), **`docs/LICENSING.md`** (**pdf-lib**). |
