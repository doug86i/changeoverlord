...
| **Patch template library** (Excel upload or duplicate; stage picks a stored template; workbook seed from API / Yjs — **`sheetsFromApiSeed`** in **`web/src/lib/patchWorkbookCollab.ts`**) | **`examples/README.md`**, **`USER_GUIDE.md`** (templates), **`api/src/routes/v1/patch-templates.ts`**, **`web/src/lib/patchWorkbookSeed.ts`**. |
