...
| **Patch template library** (Excel upload, **`POST /api/v1/patch-templates/blank`**, or duplicate; stage picks a stored template; workbook seed from API / Yjs — **`sheetsFromApiSeed`** in **`web/src/lib/patchWorkbookCollab.ts`**) | **`examples/README.md`**, **`USER_GUIDE.md`** (templates), **`api/src/routes/v1/patch-templates.ts`**, **`api/src/lib/default-patch-sheets.ts`** (must match **`patchWorkbookSeed`** expectations), **`web/src/lib/patchWorkbookSeed.ts`**. |
