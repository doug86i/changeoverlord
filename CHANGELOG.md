...
- **Web:** **Patch collab** — suppress `onOp` echo while applying remote Yjs ops. `applyOp` uses `setContextWithProduce` which re-derives patches and fires `onOp`; without suppression the echo bounced back to Yjs and the other peer applied the ops again — harmless for cell edits but created **duplicate sheets** on `addSheet`/`deleteSheet`.
