...
- **Web:** Patch / RF — collab **read-only** / hidden toolbar apply only when the viewport is **very narrow** (**≤640px**), so **iPad** and small tablets stay **editable**; **641px–768px** keeps phone-style chrome but allows edit/save. **WebSocket** pause when the tab is hidden still uses the **≤768px** “narrow” breakpoint. After remote collab ops, **`suppressLocalOps`** clears after **microtask + one** animation frame (was two rAF), reducing dropped **`onOp`** with multiple tabs.
