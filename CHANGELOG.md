...
- **Web:** **Patch collab** — skip `addSheet`/`deleteSheet` ops during opLog replay when the target sheet already exists (or is already gone) in the bootstrapped workbook. The `sheets-export` bootstrap and the Yjs opLog represent the same history; replaying structural ops on top of an already-correct initial state created **duplicate sheets**.
