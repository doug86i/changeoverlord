...
- **Web:** **Clock arena** — **Changeover / handover** banners disappeared in **very short arenas** (e.g. Event dashboard): **`min-height: 0`** on the top block let flex **collapse it to nothing** vs. the hero’s **`min-height`**. When a **`.clock-arena-handover-banner`** is present, the top block now **`flex-shrink: 0`**, **`min-height: auto`**, and a safe **`max-height`** with scroll; the hero floor is slightly lower in that case so both fit.
